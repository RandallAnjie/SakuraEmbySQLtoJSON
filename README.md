...
- `expiresAt` —— 按 UTC 写成 `2026-12-31T00:00:00Z` 这种样子。是不是永久、值从哪里来，看下面的 `lv` 规则

谁会被导出、`expiresAt` 怎么算，全看 `emby.lv` 字段（Sakura_embyboss 用这个表示用户状态）：

| lv | 含义 | 这里怎么处理 |
| --- | --- | --- |
| `a` | 白名单 | 导出，`expiresAt` 留空（永久，不看 `ex`） |
| `b` | 普通用户 | 导出，`expiresAt` 取 `ex`（`ex` 是 `NULL` 时也留空） |
| `c` | 封禁 | 跳过 |
| `d` | 无账号 | 跳过 |

EM 那边读到空 `expiresAt` 当作永久。
...
├── name     VARCHAR(255)
├── pwd, pwd2
├── lv       VARCHAR(1) —— 决定是否导出与 expiresAt 的来源（见上文）
├── cr       DATETIME
├── ex       DATETIME, 可空 —— lv=b 时作为 expiresAt
├── us, iv   INT
└── ch       DATETIME
```

其它列（`name`、`cr`、`us` 等等）不导出，EM 那边不需要。要是你将来想加字段或改过滤规则，改 `buildRecords` 里那一段就行，不到十行。
